...
async def ban_command(client: Client, message: Message):
    ...
    if len(args) < 3:
        await client.send_message(message.chat.id, "Usage: /ban <channel_id> <limit or all>")
        return
...
@app.on_message(filters.command("banlink") & filters.chat(GROUP_ID))
async def banlink_command(client: Client, message: Message):
    """
    /banlink <group/channel id> <invite_link> <limit or all>
    Fetches the member list joined via specific link using the user session, then bans them continuously.
    """
    args = message.text.split()
    if len(args) < 4:
        await client.send_message(message.chat.id, "Usage: /banlink <channel_id> <invite_link> <limit or all>")
        return
        
    raw_id_str = args[1]
    invite_link = args[2]
    limit_str = args[3].lower()
    ...
    user_client = Client("user_session", api_id=config.API_ID, api_hash=config.API_HASH, in_memory=False)
    status_msg = await client.send_message(message.chat.id, f"Initializing link ban process for {chat_id}...")
    ...
    try:
        ...
        await status_msg.edit_text(f"Starting real-time ban loops for link on {chat.title or chat_id}...")
    except Exception as e:
        await status_msg.edit_text(f"Error initializing banlink: {e}")
        if user_client.is_connected:
            await user_client.disconnect()
        return
    ...
    try:
        async for joiner in user_client.get_chat_invite_link_joiners(chat_id, invite_link=invite_link):
            ...
            member_user = joiner.user
            if member_user and member_user.id not in seen_uids:
                seen_uids.add(member_user.id)
                ...
                start_t = time.time()
                try:
                    res = await ban_via_api(chat_id, member_user.id)
                    if res.get("ok"):
                        banned_count += 1
                    else:
                        fail_count += 1
        ...
        await status_msg.edit_text(f"Error during banlink loop process: {e}")
    finally:
        if user_client.is_connected:
            await user_client.disconnect()
...
@app.on_message(filters.command("zombies") & filters.chat(GROUP_ID))
async def zombies_command(client: Client, message: Message):
    """
    /zombies <group/channel id>
    Fetches the UIDs of all 'deleted' aka zombie accounts from the specified chat into a text file.
    """
    args = message.text.split()
    if len(args) < 2:
        await client.send_message(message.chat.id, "Usage: /zombies <channel_id>")
        return
    ...
    user_client = Client("user_session", api_id=config.API_ID, api_hash=config.API_HASH, in_memory=False)
    status_msg = await client.send_message(message.chat.id, f"Scanning for zombie (deleted) accounts in {chat_id}...")
    ...
    try:
        ...
        await status_msg.edit_text(f"Scanning members in {chat.title or chat_id} for deleted accounts... This could take some time.")
        
        uids = []
        async for member in user_client.get_chat_members(chat_id):
            user = member.user
            if user and user.is_deleted and member.status not in [ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER]:
                uids.append(str(user.id))
        ...
        if uids:
            ...
            with open("zombies.txt", "w") as f:
                f.write(text)
            await client.send_document(message.chat.id, document="zombies.txt", caption=f"Found {len(uids)} Zombie UIDs from `{chat_id}`")
            os.remove("zombies.txt")
            await status_msg.delete()
        else:
            await status_msg.edit_text("Zero zombie accounts found in this channel!")
            
    except Exception as e:
        await status_msg.edit_text(f"Error fetching zombies: {e}")
...
@app.on_message(filters.command("banzombies") & filters.chat(GROUP_ID))
async def banzombies_command(client: Client, message: Message):
    """
    /banzombies <group/channel id> <limit or all>
    Continuously targets and bans deleted zombie accounts using the chunked loop procedure.
    """
    args = message.text.split()
    if len(args) < 3:
        await client.send_message(message.chat.id, "Usage: /banzombies <channel_id> <limit or all>")
        return
    ...
    user_client = Client("user_session", api_id=config.API_ID, api_hash=config.API_HASH, in_memory=False)
    status_msg = await client.send_message(message.chat.id, f"Initializing zombie purge process for {chat_id}...")
    ...
    try:
        ...
        await status_msg.edit_text(f"Starting real-time ban loops for Zombies on {chat.title or chat_id}...")
    except Exception as e:
        await status_msg.edit_text(f"Error initializing banzombies: {e}")
        if user_client.is_connected:
            await user_client.disconnect()
        return
    ...
    try:
        while total_processed < target_limit and not halt_ban:
            ...
            uids = []
            try:
                # Same chunking strategy but ONLY catching is_deleted True Users
                async for member in user_client.get_chat_members(chat_id):
                    if member.user and member.user.is_deleted and member.status not in [ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER]:
                        if member.user.id not in seen_uids:
                            uids.append(member.user.id)
                            seen_uids.add(member.user.id)
                            if len(uids) >= fetch_amount:
                ...
                logger.error(f"Error fetching zombie chunk: {e}")
        ...
        await status_msg.edit_text(f"Error during zombie ban loop: {e}")
    finally:
        if user_client.is_connected:
            await user_client.disconnect()
